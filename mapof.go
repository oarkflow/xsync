...
var _ IMap[int, string] = NewMap[int, string]()
...
// NewMap creates a new MapOf instance configured with the given
// options.
func NewMap[K comparable, V any](options ...func(*MapConfig)) *MapOf[K, V] {
...
// Get returns the value stored in the map for a key, or zero value
...
func (m *MapOf[K, V]) GetOrSet(key K, value V) (actual V, loaded bool) {
	fn := func(V, bool) (V, bool) {
		return value, false
	}
	return m.doCompute(key, fn, true, false)
}
...
// false otherwise.
func (m *MapOf[K, V]) GetAndSet(key K, value V) (actual V, loaded bool) {
	fn := func(V, bool) (V, bool) {
		return value, false
	}
	return m.doCompute(key, fn, false, false)
}
...
func (m *MapOf[K, V]) GetOrCompute(key K, valueFn func() V) (actual V, loaded bool) {
	return m.doCompute(
		key,
		func(V, bool) (V, bool) {
...
// present.
func (m *MapOf[K, V]) GetAndDelete(key K) (value V, loaded bool) {
	return m.doCompute(
		key,
			...
			for markedw != 0 {
				...
				if eptr != nil {
					...
					if e.key == key {
						...
						// snapshot won't be correct in case of multiple Set calls
						...
						// GetAndSet expects the old value to be returned.
						return oldv, true
					}
				}
				markedw &= markedw - 1
...
// ForEach calls f sequentially for each key and value present in the
// map. If f returns false, range stops the iteration.
//
// ForEach does not necessarily correspond to any consistent snapshot
// of the Map's contents: no key will be visited more than once, but
// if the value for any key is stored or deleted concurrently, ForEach
...
func (m *MapOf[K, V]) ForEach(f func(key K, value V) bool) {
